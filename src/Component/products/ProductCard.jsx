    ...
    return <>
        {isError && <>
            <div className="flex flex-col items-center justify-center  text-center gap-2  mx-auto">
                <ErrorLottie />
                <p className="text-md text-gray-500 dark:text-gray-200 font-semibold capitalize">{error.message} please try again</p>
        </div></>}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 md:gap-10   gap-6 justify-items-center">
            {allProducts?.map((pro) => (
                <Card
                    key={pro.id}
                    className="relative bg-[#f4f4f5] dark:bg-slate-800 text-[#0d4679] dark:text-white shadow-md dark:shadow-gray-50/10 rounded-2xl w-70 h-77 md:w-60 lg:w-70   overflow-hidden transform transition-transform duration-300 hover:scale-[1.03] hover:shadow-xl group py-1 cursor-pointer"
                >
                    {/* Glow effect on hover */}
                    <div className="absolute inset-0 rounded-2xl bg-gradient-to-br from-blue-400/10
                    to-blue-50/5 opacity-0 group-hover:opacity-100 blur-sm transition-all duration-300 pointer-events-none"></div>
